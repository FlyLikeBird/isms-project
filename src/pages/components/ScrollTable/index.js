...
function ScrollTable({ thead, data, scrollNum, forIndex }){
    const [scroll, setScroll] = useState({ list: data && data.length ? new Array(scrollNum).fill(1).map((item,i)=>data[i] || { empty:true }) : [], current:0});
            ...
            <div className={style['thead']}>
                {
                    ...
                    thead.map((item,index)=>(
                        <div key={index} style={{ width:item.width, fontSize:forIndex ? '0.8rem' : '1rem', color:forIndex ? '#3294d7' : '#fff', textAlign:'left' }}>{ item.title }</div>
                    ))
                    :
                    null
                }
            ...
            <div className={style['content-container']}>
                {
                    ...
                    scroll.list.map((item,index)=>(
                        <div className={style['item']} key={index} style={{ height:`${Math.floor(100/scrollNum)}%` }}>
                            {
                                thead && thead.length 
                                ?
                                thead.map((col,index)=>(
                                    <div key={index} className={col.collapse ? style['collapse'] : ''} style={{ width:col.width, fontSize:'0.8rem' }}>
                                        <span className={col.border ? IndexStyle['tag-off'] : col.dataIndex === 'rank' && !item.empty ?  style['num'] : ''}>
                                            { 
                                                item.empty 
                                                ?
                                                null
                                                ...
                                                col.key === 'time' 
                                                ? 
                                                item[col.dataIndex].substring(0, item[col.dataIndex].length - 3 ) 
                                                : 
                                                item[col.dataIndex] 
                                                ? 
                                                item[col.dataIndex] 
